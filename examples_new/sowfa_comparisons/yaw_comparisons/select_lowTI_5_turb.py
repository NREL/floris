...
yaw_cases_to_select = [np.array([0.,0.,0.,0.,0.]),
    np.array([25.,0.,0.,0.,0.]),
...
for floris_label in fi_dict:
    ...
    for i, row in df_sowfa.iterrows():
        ...
        # Save the result
        df_sowfa.at[i,floris_label] = np.round(np.array(fi.get_turbine_power())/1000.,2)
...
for idx, (i, row) in enumerate(df_sowfa.iterrows()):
    ...
    for floris_label in fi_dict:
        (fi, floris_color, floris_marker) = fi_dict[floris_label]
        ax.plot(row[floris_label],color=floris_color,marker=floris_marker,label=floris_label)
...
for floris_label in fi_dict:
    ...
    ax.plot(total,case_names,color=floris_color,marker=floris_marker,label=floris_label)
