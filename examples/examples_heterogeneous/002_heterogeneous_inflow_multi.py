...
When multiple cases are considered, the heterogeneous inflow conditions can be defined in two ways:
  ...
  1. Passing heterogenous_inflow_config to the set method, with P points,
        and speedups of size n_findex X P
  2. Assigning heterogenous_inflow_config_by_wd to the wind_data object
        used to drive FLORIS.  This object includes
        n_wd wind_directions, and speedups is of size n_wd X P.  When applied
          to set, the heterogenous_inflow_config
        is automatically generated by using the nearest wind direction
        defined in heterogenous_inflow_config_by_wd
        for each findex.
...
# If defining heterogenous_inflow_config directly, then the speedups are of size n_findex X P
...
# Now repeat using the wind_data object and heterogenous_inflow_config_by_wd
# First, create the speedups for the two wind directions
speed_ups = [[1.0, 1.25, 1.0, 1.25], [1.0, 1.35, 1.0, 1.35]]

# Create the heterogenous_inflow_config_by_wd dictionary
...
# Now create a new TimeSeries object including the heterogenous_inflow_config_by_wd
time_series = TimeSeries(
    wind_directions=np.array([269.0, 270.0, 271.0, 282.0]),
    wind_speeds=8.0,
    turbulence_intensities=0.06,
