...
yaw_1 = st.sidebar.slider("1T: Yaw angle", -30.0, 30.0, 0.0, step=1.0)
x_loc_1 = st.sidebar.slider("1T: D downstream", 2.0, 30.0, 7.0, step=1.0)

# Options for 2 turbine case
x_loc_2 = st.sidebar.slider("2T: D downstream", 2.0, 30.0, 7.0, step=1.0)
spacing_2 = st.sidebar.slider("2T: D Spacing", 2.0, 12.0, 7.0, step=1.0)
offset_2 = st.sidebar.slider("2T: D Offset", -2.0, 2.0, 0.0, step=0.25)

# Options for deep array case
x_loc_3 = st.sidebar.slider("Deep: D downstream", 2.0, 50.0, 7.0, step=1.0)
n_row = st.sidebar.slider("Deep: Num Rows", 2, 12, 2, step=1)
spacing_3 = st.sidebar.slider("Deep: D Spacing", 2.0, 12.0, 7.0, step=1.0)
offset_3 = st.sidebar.slider("Deep: D Offset", -2.0, 2.0, 0.0, step=0.25)
...
for i in range(num_models):
    ...
    for y_idx, y_loc in enumerate(sweep_locations):
            ...
            layout_array=([0, t1_x_loc, sweep_loc], [0, t1_y_loc, y_loc * D],)
        )
...
for i in range(num_models):
    ...
    # Configure model
    fi.reinitialize_flow_field(wind_speed=ws, layout_array=[deep_x, deep_y])
