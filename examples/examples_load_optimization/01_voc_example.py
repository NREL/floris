...
from floris.optimization.load_optimization.load_optimization import compute_turbine_voc
...
for t in range(fmodel.n_turbines):
    ax.plot(wind_directions, voc[:, t], label=f"Turbine {t}")
ax.set_ylabel("VOC")
...
fig, ax = plt.subplots()
for t in range(fmodel.n_turbines):
    ax.plot(power_setpoints, voc[:, t], label=f"Turbine {t}")
ax.set_ylabel("VOC")
