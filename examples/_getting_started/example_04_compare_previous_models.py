...
fi_no_gch = wfct.floris_interface.FlorisInterface("../example_input.json")
fi_gch = wfct.floris_interface.FlorisInterface("../example_input.json")

# Turn off gch
fi_no_gch.set_gch(False)
...
for idx, (fi, name) in enumerate(
    zip(
        [fi_jensen, fi_turbopark, fi_mz, fi_no_gch, fi_gch],
        ["Jensen", "TurbOPark", "Multizone", "Gaussian (No GCH)", "GCH"],
    )
):
    ...
    wfct.visualization.visualize_cut_plane(hor_plane, ax=ax, minSpeed=5, maxSpeed=8)
    ax.set_title(name)
