...
class WindRose:
    ...
    def resample_average_ws_by_wd(self, df):
        ...
        for val in df.wd.unique():
            ws_avg.append(
                np.array(df.loc[df["wd"] == val]["ws"] * df.loc[df["wd"] == val]["freq_val"]).sum()
    ...
    def interpolate(
        ...
        interp = interpolator(points=df[["wd", "ws"]], values=df["freq_val"], fill_value=fill_value)
    ...
    def make_wind_rose_from_weibull(self, wd=np.arange(0, 360, 5.0), ws=np.arange(0, 26, 1.0)):
        """
    ...
    def read_wind_rose_csv(self, filename):
        # Read in the csv
    ...
    def import_from_wind_toolkit_hsds(
        ...
        # Case for ht not matching discete height
            ...
            ws_new = np.array(ws_low) * (1 - ((ht - h_low) / (h_up - h_low))) + np.array(
                ws_high
            ) * ((ht - h_low) / (h_up - h_low))
