...
def energy_ratio_region(ref_pow_base, test_pow_base, ws_base, wd_base,
    ...
    tuple_unique = np.array([
        x for x in set(tuple(x) for x in tuple_unique_base)
        & set(tuple(x) for x in tuple_unique_con)
    ])
    ...
    tuples_unique_base, counts_base = np.unique(base_tuples,
                                                axis=0,
                                                return_counts=True)
    tuples_unique_con, counts_con = np.unique(con_tuples,
                                              axis=0,
                                              return_counts=True)
    ...
    weights_base_LUT = dict(
        zip(tuple(map(tuple, tuples_unique_base)), weights_base))
    weights_con_LUT = dict(
        zip(tuple(map(tuple, tuples_unique_con)), weights_con))
...
def calculate_balanced_energy_ratio_region(reference_power_baseline,
    ...
    wind_direction_array_baseline = wind_direction_array_baseline.round(
    ).astype(int)
    wind_speed_array_controlled = wind_speed_array_controlled.round().astype(
        int)
    wind_direction_array_controlled = wind_direction_array_controlled.round(
    ).astype(int)

    # Call the energy ratio for the region
    ratio_base, ratio_con, ratio_diff, p_change, counts_base, counts_con, counts_diff, counts_pchange = energy_ratio_region(
        reference_power_baseline, test_power_baseline,
        wind_speed_array_baseline, wind_direction_array_baseline,
        reference_power_controlled, test_power_controlled,
        wind_speed_array_controlled, wind_direction_array_controlled)
    ...
    for i_bs in range(n_boostrap):
        ...
        ind_bs = np.random.randint(len(reference_power_baseline),
                                   size=len(reference_power_baseline))
        ...
        ind_bs = np.random.randint(len(reference_power_controlled),
                                   size=len(reference_power_controlled))
        ...
        wind_direction_binned_controlled = wind_direction_array_controlled[
            ind_bs]

        # compute the energy ratio
        ratio_base_bs[i_bs], ratio_con_bs[i_bs], diff_bs[i_bs], p_change_bs[
            i_bs], _, _, _, _ = energy_ratio_region(
                reference_power_binned_baseline, test_power_binned_baseline,
                wind_speed_binned_baseline, wind_direction_binned_baseline,
                reference_power_binned_controlled,
                test_power_binned_controlled, wind_speed_binned_controlled,
                wind_direction_binned_controlled)
    ...
    lower_ratio_base, upper_ratio_base = _calculate_lower_and_upper_bound(
        ratio_base_bs,
        percentiles,
        central_estimate=ratio_base,
        method='simple_percentile')
    lower_ratio_con, upper_ratio_con = _calculate_lower_and_upper_bound(
        ratio_con_bs,
        percentiles,
        central_estimate=ratio_con,
        method='simple_percentile')
    lower_diff, upper_diff = _calculate_lower_and_upper_bound(
        diff_bs,
        percentiles,
        central_estimate=ratio_diff,
        method='simple_percentile')
    lower_p_change, upper_p_change = _calculate_lower_and_upper_bound(
        p_change_bs,
        percentiles,
        central_estimate=p_change,
        method='simple_percentile')

    print(ratio_base, lower_ratio_base, upper_ratio_base, counts_base,
          ratio_con, lower_ratio_con, upper_ratio_con, counts_con, ratio_diff,
          lower_diff, upper_diff, counts_diff, p_change, lower_p_change,
          upper_p_change, counts_pchange)
...
def calculate_balanced_energy_ratio(
    ...
    for i, wind_direction_bin in enumerate(wind_direction_bins):
        ...
        for i_bs in range(n_boostrap):
            ...
            # random resampling w/ replacement
            ind_bs = np.random.randint(len(reference_power_baseline_wd),
            ...
            ind_bs = np.random.randint(len(reference_power_controlled_wd),
