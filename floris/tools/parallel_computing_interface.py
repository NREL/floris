...
def _load_local_floris_object(fi_dict, het_map=None, unc_pmfs=None, fix_yaw_in_relative_frame=False):
...
class ParallelComputingInterface(LoggerBase):
            ...
            [
                np.concatenate(  # Merges wind directions
                    [
                        ...
                        for f in subset[wii * self.n_wind_direction_splits : (wii + 1) * self.n_wind_direction_splits]
                    ],
                    axis=0,
                )
    ...
    def _postprocessing(self, output):
        ...
        turbine_powers = np.concatenate(
            [
                np.concatenate(
                    power_subsets[self.n_wind_speed_splits * (ii) : self.n_wind_speed_splits * (ii + 1)], axis=1
                )
    ...
    def get_turbine_powers(self, yaw_angles=None):
        ...
        if self.print_timings:
            print("==================================================================================")
            print(
                "Total time spent for parallel calculation ({:d} workers): {:.3f} s".format(self.max_workers, t_total)
            )
    ...
    def get_farm_power(self, yaw_angles=None, turbine_weights=None):
        if turbine_weights is None:
            ...
            turbine_weights = np.tile(
                turbine_weights,
                (self.fi.floris.flow_field.n_wind_directions, self.fi.floris.flow_field.n_wind_speeds, 1),
            )
    ...
    def optimize_yaw_angles(
        ...
        if self.print_timings:
            print("==================================================================================")
            print(
                "Total time spent for parallel calculation ({:d} workers): {:.3f} s".format(self.max_workers, t3 - t0)
            )
