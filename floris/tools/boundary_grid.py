...
def discontinuous_grid(nrows,ncols,farm_width,farm_height,shear,rotation,center_x,center_y,shrink_boundary,boundary_x,boundary_y,eps=1E-3):
    """
    ...
    # rotate
    grid_x, grid_y = np.cos(rotation)*grid_x - np.sin(rotation)*grid_y, np.sin(rotation)*grid_x + np.cos(rotation)*grid_y
    ...
    if shrink_boundary != 0.0:
        ...
        for i in range(nBounds):
            ...
            if poly.contains(point)==True or poly.touches(point)==True:
    ...
    for i in range(len(grid_x)):
        ...
        if poly.contains(point)==False and poly.touches(point)==False:
    ...
    for i in range(n_boundary_turbs):
        for j in range(nBounds-1):
            if flattened_locs[i] < sum(lenBound[0:j+1]):
                layout_x[i] = boundary_x[j] + (boundary_x[j+1]-boundary_x[j])*(flattened_locs[i]-sum(lenBound[0:j]))/lenBound[j]
                layout_y[i] = boundary_y[j] + (boundary_y[j+1]-boundary_y[j])*(flattened_locs[i]-sum(lenBound[0:j]))/lenBound[j]
                break
    
    return layout_x, layout_y


def boundary_grid(n_boundary_turbs,start,nrows,ncols,farm_width,farm_height,shear,rotation,center_x,center_y,shrink_boundary,boundary_x,boundary_y,eps=1E-3):
    """
    Place turbines equally spaced traversing the perimiter if the wind farm along the boundary
    ...
    boundary_turbines_x, boundary_turbines_y = place_boundary_turbines(n_boundary_turbs,start,boundary_x,boundary_y) 
    grid_turbines_x, grid_turbines_y = discontinuous_grid(nrows,ncols,farm_width,farm_height,shear,rotation,center_x,center_y,shrink_boundary,boundary_x,boundary_y,eps=eps)
...
class BoundaryGrid:
    ...
    def reinitialize_xy(self):
        ...
        layout_x,layout_y = boundary_grid(self.n_boundary_turbs,self.start,self.nrows,self.ncols,self.farm_width,self.farm_height,self.shear,
                            self.rotation,self.center_x,self.center_y,self.shrink_boundary,self.boundary_x,self.boundary_y,eps=self.eps)
...
if __name__=="__main__":
    ...
    boundary_x = np.array([0.0,100.0,100.0,200.0,200.0,300.0,300.0,400.0,400.0,500.0,500.0,0.0])
    boundary_y = np.array([500.0,500.0,400.0,400.0,300.0,300.0,200.0,200.0,100.0,100.0,600.0,600.0])
    ...
    x,y = discontinuous_grid(nrows,ncols,farm_width,farm_height,shear,rotation,center_x,center_y,shrink_boundary,boundary_x,boundary_y)


    n_boundary_turbs = 25
    start = 1000.0
    layout_x, layout_y = place_boundary_turbines(n_boundary_turbs,start,boundary_x,boundary_y)
    ...
    nx, ny = boundary_grid(n_boundary_turbs,start,nrows,ncols,farm_width,farm_height,shear,rotation,center_x,center_y,shrink_boundary,boundary_x,boundary_y)
