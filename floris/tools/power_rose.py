...
class PowerRose():
    ...
    def _all_combine(self):
        ...
        df_turbine_power_no_wake = self.df_turbine_power_no_wake.copy(deep=True)
        df_turbine_power_baseline = self.df_turbine_power_baseline.copy(deep=True)
        df_turbine_power_opt = self.df_turbine_power_opt.copy(deep=True)

        # Adjust the column names for uniqunes
        df_yaw.columns = ['yaw_%d' % c if type(c) is int else c for c in df_yaw.columns] 
        df_turbine_power_no_wake.columns = ['tnw_%d' % c if type(c) is int else c for c in df_turbine_power_no_wake.columns] 
        df_turbine_power_baseline.columns = ['tb_%d' % c if type(c) is int else c for c in df_turbine_power_baseline.columns] 
        df_turbine_power_opt.columns = ['topt_%d' % c if type(c) is int else c for c in df_turbine_power_opt.columns] 

        # Merge
        df_combine = df_power.merge(df_yaw,on=['ws','wd'])
        df_combine = df_combine.merge(df_turbine_power_no_wake,on=['ws','wd'])
        df_combine = df_combine.merge(df_turbine_power_baseline,on=['ws','wd'])
    ...
    def _compute_energy(self):
        self.df_power['energy_no_wake'] = self.df_power.freq_val * self.df_power.power_no_wake
        self.df_power['energy_baseline'] = self.df_power.freq_val * self.df_power.power_baseline
        self.df_power['energy_opt'] = self.df_power.freq_val * self.df_power.power_opt
    ...
    def report(self):
        ...
        print('---------------------------------------------')
        print('Energy\t%.1E\t\t%.1E\t\t%.1E' % (self.total_no_wake,self.total_baseline,self.total_opt ))
        print('%%\t--\t\t%.1f%%\t\t%.1f%%' % (100. * self.baseline_percent,100. * self.opt_percent ))
        print('Wk Loss\t--\t\t%.1f%%\t\t%.1f%%' % (100. * self.baseline_wake_loss,100. * self.opt_wake_loss ))
        print('AEP Gain --\t\t--\t\t%.1f%%' % (100. * self.percent_gain ))
        print('Loss Red --\t\t--\t\t%.1f%%' % (100. * self.reduction_in_wake_loss ))

    def initialize(self, name,df_power, df_yaw, df_turbine_power_no_wake, df_turbine_power_baseline,df_turbine_power_opt):
    ...
    def load(self, filename):
        self.name, self.df_power, self.df_yaw, self.df_turbine_power_no_wake, self.df_turbine_power_baseline, self.df_turbine_power_opt,self.df_combine = pickle.load( open( filename, "rb" ) )
    ...
    def save(self, filename):
        pickle.dump( [self.name, self.df_power, self.df_yaw, self.df_turbine_power_no_wake, self.df_turbine_power_baseline, self.df_turbine_power_opt, self.df_combine], open( filename, "wb" ) )
    ...
    def plot_by_direction(self):
        ...
        ax = axarr[0]
        ax.plot(df.wd, df.energy_baseline / df.energy_no_wake, label='Baseline',color='k')
        ax.axhline(np.mean(df.energy_baseline) / np.mean(df.energy_no_wake), color='k',ls='--')
        ...
        ax = axarr[1]
        ax.plot(df.wd, 100. * (df.energy_opt - df.energy_baseline) / df.energy_baseline,'r')
        ax.axhline(100. * (df.energy_opt.mean() - df.energy_baseline.mean()) / df.energy_baseline.mean(),color='r',ls='--')
    ...
    def wake_loss_at_direction(self, wd):
        ...
        ax = axarr[1,0]
        ax.plot(df.ws,df.power_baseline/df.power_no_wake,'b',label='Baseline')
        ax.plot(df.ws,df.power_opt/df.power_no_wake,'r',label='Opt')
        ax.set_ylabel('Percent')
        ax.grid()

        ax = axarr[1,1]
        ax.plot(df.ws,df.energy_baseline/df.energy_no_wake,'b',label='Baseline')
        ...
        ax = axarr[3,0]
        ax.plot(df.ws,(df.power_opt - df.power_baseline)/df.power_baseline,'r')
        ax.set_ylabel('Percent Gain')
        ax.grid()
        ax.set_xlabel('Wind Speed (m/s)')

        ax = axarr[3,1]
        ax.plot(df.ws,(df.energy_opt - df.energy_baseline)/df.energy_baseline,'r')
        ax.grid()
        ax.set_xlabel('Wind Speed (m/s)')
