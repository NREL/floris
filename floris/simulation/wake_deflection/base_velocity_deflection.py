...
class VelocityDeflection():
    ...
    def calculate_effective_yaw_angle(self, x_locations, y_locations, z_locations, turbine, coord, flow_field):
        if self.use_ss:
            # turbine parameters
            Ct = turbine.Ct
            ...
            Gamma = V[idx] * ((2 * np.pi) * (yLocs[idx]**2 + zLocs[idx]**2)) / (yLocs[idx] * (1 - np.exp(-(yLocs[idx]**2 + zLocs[idx]**2) / ((eps)**2))))
            ...
            try:
                ...
                # TODO: should this really be handled or let the error throw when this doesnt work?
                print('ERROR', idx)
                yaw_effective = 0.0
