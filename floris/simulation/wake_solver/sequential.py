...
class Sequential(WakeSolver):
    """
    The sequential solver does some things.
    ...
    def function(
        ...
        for coord, turbine in sorted_map:
            ...
            (
                ...
                rotated_x, rotated_y, rotated_z, turbine, coord, deflection, flow_field
            )
            ...
            if not no_wake:
                u_wake = flow_field.wake.combination_function(u_wake, turb_u_wake)

                if flow_field.wake.velocity_model.model_string == "curl":
                    flow_field.v = turb_v_wake
                    flow_field.w = turb_w_wake
                else:
